...
test("Issue 3 — breakDurationMinutes: بيفضّل break_mins الجاهز لو موجود", () => {
  const b: BreakRecord = {
    ...
    end_time: "2026-08-13T10:20:00Z",
    break_mins: 15,
  };
  assert.equal(
    breakDurationMinutes(b),
    15,
    "المفروض يرجع break_mins مباشرة مش يحسبها من الفرق",
  );
});
...
test("Issue 3 — summarizeBreaks: بيجمع أكتر من بريك لنفس الموظف في نفس اليوم (مش بريك واحد بس)", () => {
  const breaks: BreakRecord[] = [
    ...
    },
    {
      ...
      updated_at: "",
      attendance_id: 10,
      start_time: "2026-08-13T15:00:00Z",
      end_time: "2026-08-13T15:05:00Z",
      break_mins: 5,
    },
  ];
...
test("Issue 3 — getBreaksSummaryByAttendanceIds (الدالة اللي كانت ناقصة أصلاً): بترجع Map صح لكل سجل", async () => {
  const allBreaks: BreakRecord[] = [
    ...
    },
    {
      ...
      updated_at: "",
      attendance_id: 100,
      start_time: "2026-08-10T13:00:00Z",
      end_time: "2026-08-10T13:10:00Z",
      break_mins: 10,
    },
    {
      id: 3,
...
test("getCurrentOpenBreak: بيرجع البريك المفتوح لو واحد بس", () => {
  const breaks: BreakRecord[] = [
    {
      id: 1,
      created_at: "",
  ...
  assert.equal(getCurrentOpenBreak(breaks)?.id, 2);
});

test("getCurrentOpenBreak: مفيش بريك مفتوح خالص → null", () => {
  const breaks: BreakRecord[] = [
    {
      id: 1,
      created_at: "",
  ...
  assert.equal(getCurrentOpenBreak(breaks), null);
});

test("getCurrentOpenBreak: فيه بريك يتيم (orphan) قديم + بريك حالي — بياخد الأحدث بدأ", () => {
  const breaks: BreakRecord[] = [
    {
      id: 1,
      created_at: "",
  ...
  assert.equal(
    getCurrentOpenBreak(breaks)?.id,
    2,
    "المفروض ياخد الأحدث start_time مش أول عنصر في المصفوفة",
  );
});

test("computeTotalBreakSeconds: بريك يتيم مفتوح متجاهل، مش بيتضاف breakElapsedSec عليه بالغلط", () => {
  const breaks: BreakRecord[] = [
    {
      id: 1,
      created_at: "",
  ...
  const breakElapsedSec = 120; // دقيقتين بيعد فيهم البريك الحالي
  const total = computeTotalBreakSeconds(breaks, breakElapsedSec);
  // المفروض: 10 دقايق (600 ثانية) + 120 ثانية للحالي فقط = 720
  // مش orphan + الحالي مع بعض (اللي كان بيحصل في الباگ القديم)
  assert.equal(
    total,
    720,
    "الـ orphan لازم يتجاهل تمامًا، مش يتضاف عليه breakElapsedSec زي البريك الحالي",
  );
});

test("computeTotalBreakSeconds: مفيش أي بريك مفتوح → مجموع البريكات المقفولة بس", () => {
  const breaks: BreakRecord[] = [
    {
      id: 1,
      created_at: "",
  ...
  assert.equal(
    computeTotalBreakSeconds(breaks, 999),
    1800,
    "breakElapsedSec متلمهوش لأن مفيش بريك مفتوح خالص",
  );
});
...
test("Issue 2 — محاكاة: أكشن على صف تاني وقت ما صف مشغول لازم يتمنع (busyLeaveId guard)", () => {
  ...
  assert.deepEqual(
    callLog,
    [1],
    "أكشن التاني كان المفروض يترفض لحد ما الأول يخلص",
  );
...
test("Issue 2 — كل أكشن بيمسح الـ error بتاعه في try/catch جديد (مفيش error عالق من عملية قبله)", async () => {
  ...
  async function runAction(shouldFail: boolean) {
    try {
      if (shouldFail)
        throw new Error("Only accepted leaves can be ended early");
      toasts.push({ type: "success", msg: "تم إنهاء الإجازة بدري" });
    } catch (err) {
      toasts.push({
        type: "error",
        msg: err instanceof Error ? err.message : "تعذر إنهاء الإجازة",
      });
    }
  }
  ...
  assert.equal(
    toasts[1].type,
    "success",
    "التوست التاني المفروض يبقى success نضيف، مش error عالق من قبله",
  );
});
...
test("فيكس getMyAttendanceToday — لو فيه أكتر من سجل حضور لنفس اليوم، بياخد الأحدث بدل ما ينفجر", () => {
  ...
  assert.equal(
    latest?.id,
    502,
    "المفروض ياخد آخر سجل حسب check_in_at مش يرمي error زي .maybeSingle() القديمة",
  );
});
...
test("dedupeAttendanceTodayByUser: بياخد آخر سجل لكل موظف (مش بيكرر users_id في الجدول)", () => {
  const rows: AttendanceTodayRow[] = [
    {
      users_id: "u1",
      name: "أحمد",
      check_in_at: "2026-08-13T08:00:00Z",
      check_out_at: null,
      late_minutes: 0,
      status: "present",
    },
    {
      users_id: "u1",
      name: "أحمد",
      check_in_at: "2026-08-13T08:30:00Z",
      check_out_at: null,
      late_minutes: 30,
      status: "late",
    },
    {
      users_id: "u2",
      name: "سارة",
      check_in_at: "2026-08-13T07:55:00Z",
      check_out_at: null,
      late_minutes: 0,
      status: "present",
    },
  ...
  assert.equal(
    ...
    "المفروض ياخد آخر check_in_at لأحمد",
  );
});

test("resolveStatus: بيرجع status من الباك لو معروف", () => {
  const r: AttendanceTodayRow = {
    users_id: "u1",
    name: "أحمد",
    check_in_at: "2026-08-13T08:00:00Z",
    check_out_at: null,
...
test("resolveStatus: status غير معروف/null بس فيه check_in + تأخير → late", () => {
  const r: AttendanceTodayRow = {
    users_id: "u1",
    name: "أحمد",
    check_in_at: "2026-08-13T09:30:00Z",
    check_out_at: null,
    late_minutes: 15,
    status: null,
  };
